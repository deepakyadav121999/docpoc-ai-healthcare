...
import ReportGenerateAgent from "./ReportGenerateAgent";
...
const AIChatInterface = () => {
  ...
  const [showReportAgent, setShowReportAgent] = useState(false);
  ...
  useEffect(() => {
    // Only start a new chat if we're on the home page and there's no session
    if (
      !sessionId &&
      ...
      console.log(
        "AIChatInterface: No sessionId found, starting new chat automatically",
      );
      startNewChat();
    }
  ...
  const handleGenerateReport = async (input: string, files: File[]) => {
    setIsGeneratingReport(true);

    try {
      // Create user message for the report request
      const userMessage: Message = {
        id: new Date().toISOString(),
        text: `Generate a report based on: ${input}${files.length > 0 ? ` (with ${files.length} file(s) uploaded)` : ""}`,
        sender: "user",
        timestamp: new Date().toISOString(),
      };
      setMessages((prevMessages) => [...prevMessages, userMessage]);

      // Simulate report generation with streaming
      const reportResponse = `# 📊 Generated Report\n\nBased on your request "${input}", here's a comprehensive report:\n\n## Executive Summary\nThis report has been generated using advanced AI analysis of your provided information and documents.\n\n## Key Findings\n- Data analyzed from ${files.length > 0 ? `${files.length} uploaded file(s)` : "text input"}\n- Insights extracted and processed\n- Recommendations formulated\n\n## Detailed Analysis\n\n### Input Analysis\nYour request: "${input}"\n\n### Document Processing\n${files.length > 0 ? `Successfully processed ${files.length} file(s):\n${files.map((f) => `- ${f.name} (${(f.size / 1024 / 1024).toFixed(2)} MB)`).join("\n")}` : "No additional files provided"}\n\n### Generated Insights\n1. **Primary Focus**: Healthcare report generation\n2. **Data Sources**: ${files.length > 0 ? "Uploaded documents and text input" : "Text input only"}\n3. **Analysis Method**: AI-powered content analysis\n\n## Recommendations\n1. Review the generated insights\n2. Validate findings with your team\n3. Implement suggested actions\n4. Schedule follow-up analysis if needed\n\n## Next Steps\n- Share this report with relevant stakeholders\n- Archive for future reference\n- Update your records as needed\n\n---\n*Report generated by Doku Healthcare AI Assistant*`;
      ...
      await simulateStreamingResponse(reportResponse, (streamingText) => {
        // setStreamingMessage(streamingText);
        setMessages((prevMessages) =>
          prevMessages.map((msg) =>
            msg.id === streamingMessageId
      ...
      setShowReportAgent(false);
    }
  };

  const MessageBubble = ({ msg }: { msg: Message }) => {
    const isCurrentStreaming =
      isStreaming && msg.sender === "doku" && msg.id.startsWith("streaming-");
  ...
  return (
    ...
    >
      ...
      {/* Messages Area - Only show in Chat Mode */}
      {!showReportAgent && (
            ...
            {messages.length === 0 && !isThinking ? (
              <div className="text-center py-12">
                ...
                <p className="text-gray-600 dark:text-gray-400 mb-8">
                  I can help you with creating patients, appointments, reports,
                  and sending WhatsApp reminders. How can I help you today?
                </p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-2xl mx-auto">
                  ...
                  <button
                    onClick={() => setShowReportAgent(true)}
                    className="p-4 text-left bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 hover:border-blue-300 dark:hover:border-blue-600 transition-colors"
                  >
                    <h3 className="font-medium text-gray-900 dark:text-white mb-1">
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-400">
                      Generate reports from text or images
                    </p>
                  </button>
                  <button
      ...
      <div className="flex-shrink-0 bg-white dark:bg-gray-800 px-2 sm:px-4 lg:px-6 py-2 border-b border-gray-200 dark:border-gray-700">
        <div className="max-w-4xl mx-auto">
          <div className="flex gap-1 sm:gap-2">
            <button
              onClick={() => setShowReportAgent(false)}
              className={`flex-1 flex items-center justify-center gap-1 sm:gap-2 px-2 sm:px-4 py-2 rounded-md sm:rounded-lg font-medium transition-all duration-200 ${
                !showReportAgent
            ...
            >
              ...
              <span className="text-xs sm:text-sm truncate">Chat Mode</span>
            </button>
            <button
              onClick={() => setShowReportAgent(true)}
              className={`flex-1 flex items-center justify-center gap-1 sm:gap-2 px-2 sm:px-4 py-2 rounded-md sm:rounded-lg font-medium transition-all duration-200 ${
          ...
          <div className="max-w-4xl mx-auto">
            ...
            <div className="relative">
              <Input
                ...
                classNames={{
                  base: "w-full",
                  input: "pr-28 sm:pr-36 text-sm sm:text-base",
                  inputWrapper:
                    "rounded-2xl border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 hover:border-gray-400 dark:hover:border-gray-500 focus-within:border-blue-500 focus-within:ring-2 focus-within:ring-blue-500/20",
                }}
              />
              <div className="absolute top-1/2 right-2 -translate-y-1/2 flex items-center gap-1 sm:gap-2">
                ...
                <Button
                  ...
                  isDisabled={!inputValue.trim() && uploadedFiles.length === 0}
                  isIconOnly
                  color="primary"
                  size="sm"
            ...
            <div className="flex items-center justify-between mt-2 text-xs text-gray-500 dark:text-gray-400">
              <span className="hidden sm:inline">
                Press Enter to send, Shift+Enter for new line • Ask about
                healthcare tasks
              </span>
              <span className="sm:hidden">
                Press Enter to send • Healthcare AI
              </span>
              {listening && (
