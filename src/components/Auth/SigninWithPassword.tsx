...
export default function SigninWithPassword({ onLogin }: { onLogin: (token: string) => void }) {
  const router = useRouter();

  // State management
  const [loginMethod, setLoginMethod] = useState<"password" | "otp">("password"); // Toggle between password and OTP login
  ...
  const [isOtpFieldVisible, setIsOtpFieldVisible] = useState(false); // To control OTP field visibility
  const timerCount = 30;
  ...
  async function sendOtp() {
    ...
    try {
      await axios.post("http://127.0.0.1:3037/DocPOC/v1/auth/otp/generate", { phone });
      setTimer(timerCount); // Start the OTP resend timer
      setIsOtpFieldVisible(true); // Show OTP input field
    } catch (err) {
  ...
  async function handleOtpVerification(event: React.FormEvent) {
    ...
    try {
      const response = await axios.post("http://127.0.0.1:3037/DocPOC/v1/auth/otp/verify", {
        phone,
        otp,
      });
  ...
  return (
    <div>
      ...
      <div className="flex justify-center mb-6">
        <button
          className={`mr-4 ${loginMethod === "password" ? "font-bold underline" : ""}`}
          onClick={() => setLoginMethod("password")}
        >
          Login with Password
        </button>
        <button
          className={`${loginMethod === "otp" ? "font-bold underline" : ""}`}
      ...
      {loginMethod === "otp" && (
        <form onSubmit={handleOtpVerification}>
          ...
          {/* Dynamic Button: Send OTP or Verify OTP */}
          {/* <StyledButton
            label={
              loading
                ? isOtpFieldVisible
            ...
            style={{ width: "100%" }}
            onClick={!isOtpFieldVisible ? sendOtp : undefined}
            loading={loading}
          /> */}
          ...
          {isOtpFieldVisible && (
            <button
              ...
              disabled={timer > 0}
              onClick={sendOtp}
              className={`mt-3 w-full text-center p-2 ${timer > 0 ? "text-gray-500" : "text-blue-500"}`}
            >
