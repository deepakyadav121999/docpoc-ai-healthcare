...
//                 {/* Calendar Grid */}
//                 <div className="grid grid-cols-7 gap-0.5 sm:gap-0.5 flex-1">
...
> = ({
  ...
  // Add CSS to ensure calendar positioning is independent
  useEffect(() => {
      ...
      .calendar-dropdown {
       ...
       .calendar-dropdown button {
         ...
         overflow: hidden !important;
             ...
             /* Responsive heights for different screen sizes - optimized for better space utilization */
  ...
  const [dropdownPosition, setDropdownPosition] = useState({
    ...
    placement: "bottom" as "top" | "bottom",
    horizontalAlignment: "left" as "left" | "right",
    height: window.innerWidth < 640 ? 320 : window.innerWidth < 768 ? 380 : 450, // Optimized height for better space utilization
    width: 320,
    isPositioned: false,
  });
  const [inputValue, setInputValue] = useState(
  ...
  // Enhanced dropdown position calculation for modals
  const calculateDropdownPosition = () => {
    if (!pickerRef.current || !inputRef.current) return;
    ...
    // Enhanced modal/container detection
    let containerRect = {
      top: 0,
      left: 0,
      right: viewportWidth,
    ...
    const modalContainer = pickerRef.current.closest(
      '.modal, .dialog, [role="dialog"], .fixed, .absolute, [data-slot="base"], .nextui-modal-wrapper, .nextui-modal',
    );

    if (modalContainer) {
      const modalRect = modalContainer.getBoundingClientRect();
      containerRect = {
        top: Math.max(0, modalRect.top + 10), // Add some padding
        left: Math.max(0, modalRect.left + 10),
        right: Math.min(viewportWidth, modalRect.right - 10),
        bottom: Math.min(viewportHeight, modalRect.bottom - 10),
      };
    }

    // Responsive dropdown dimensions with optimized height for better space utilization
    const isSmallScreen = viewportWidth < 640;
    const isMediumScreen = viewportWidth < 768;

    // Optimized height based on screen size for better space utilization
    const dropdownHeight = isSmallScreen ? 320 : isMediumScreen ? 380 : 450; // Better space utilization
    let dropdownWidth = isSmallScreen ? 260 : isMediumScreen ? 300 : 320;

    const buffer = 8;
    const gap = 4;

    // Calculate available space within container (only for width, not height)
    // const containerHeight = containerRect.bottom - containerRect.top;
    const containerWidth = containerRect.right - containerRect.left;

    // Only adjust width to fit container, keep height fixed
    const maxWidth = containerWidth - 20; // Leave some side padding

    dropdownWidth = Math.min(dropdownWidth, Math.max(240, maxWidth));

    // Available spaces within container
    const spaceBelow = containerRect.bottom - inputRect.bottom - buffer;
    const spaceAbove = inputRect.top - containerRect.top - buffer;
    const spaceRight = containerRect.right - inputRect.left - buffer;
    const spaceLeft = inputRect.right - containerRect.left - buffer;

    // Determine vertical placement - height is fixed, so just check if there's enough space
    ...
    if (spaceBelow >= dropdownHeight) {
      ...
      // If neither side has enough space, still use fixed height but position where there's more space
      placement = spaceBelow >= spaceAbove ? "bottom" : "top";
      // Keep dropdownHeight fixed at 400px - don't reduce it
    }

    // Determine horizontal alignment
    let horizontalAlignment: "left" | "right" = "left";
    if (spaceRight >= dropdownWidth) {
      horizontalAlignment = "left";
    } else if (spaceLeft >= dropdownWidth) {
      horizontalAlignment = "right";
    } else {
      // Adjust width and choose better side
      const availableSpace = Math.max(spaceRight, spaceLeft);
      dropdownWidth = Math.max(
        220,
        Math.min(dropdownWidth, availableSpace - gap),
      );
      horizontalAlignment = spaceRight >= spaceLeft ? "left" : "right";
    }
    ...
    if (placement === "bottom") {
      ...
      top = inputRect.top - dropdownHeight - gap;
    }
    ...
    left = Math.max(
      ...
      Math.min(left, containerRect.right - dropdownWidth - buffer),
    );

    top = Math.max(
      containerRect.top + buffer,
      Math.min(top, containerRect.bottom - dropdownHeight - buffer),
    );

    // Final boundary checks - height is fixed, only adjust position if needed
    if (top + dropdownHeight > containerRect.bottom - buffer) {
      top = containerRect.bottom - dropdownHeight - buffer;
      if (top < containerRect.top + buffer) {
  ...
  // Immediate position calculation without delay
  useEffect(() => {
    if (!isOpen) {
      setDropdownPosition((prev) => ({ ...prev, isPositioned: false }));
      return;
    }
    ...
    const handleResize = () => {
      // Update height based on current screen size with optimized values
      const newHeight =
        window.innerWidth < 640 ? 320 : window.innerWidth < 768 ? 380 : 450;
      setDropdownPosition((prev) => ({ ...prev, height: newHeight }));
    ...
    const handleScroll = (e: Event) => {
      ...
      const target = e.target as Node | null;
      // Only reposition if scroll is not from within our components
      if (
        target &&
          ...
          <div className="w-7 h-7">
            <div className="w-7 h-7 rounded flex items-center justify-center">
              <svg
                ...
                width="100%"
                viewBox="0 0 106 85"
                enableBackground="new 0 0 106 85"
                className={disabled ? "text-gray-400 dark:text-gray-600" : ""}
              >
                <path
                  fill="#6B46C1"
...
z"
      ...
      {isOpen && !disabled && (
        <div
          ref={dropdownRef}
          className={`calendar-dropdown fixed bg-white dark:bg-gray-dark border border-gray-200 dark:border-gray-600 rounded-xl shadow-xl dark:shadow-2xl overflow-hidden transition-all duration-200 ${
            dropdownPosition.isPositioned
          ...
          style={{
            ...
            zIndex: 99999,
            width: `${dropdownPosition.width}px`,
            height: `${dropdownPosition.height}px`, // Use responsive height from state
            transformOrigin:
              dropdownPosition.placement === "bottom" ? "top" : "bottom",
          }}
        >
          {/* Arrow indicator */}
          <div
            ...
            style={{
              left: Math.min(
                Math.max(20, dropdownPosition.width * 0.15),
                dropdownPosition.width - 32,
              ),
            }}
          />

          {/* Header - Fixed height */}
          <div className="flex-shrink-0 p-1 md:p-2 bg-gradient-to-r from-purple-50 to-purple-100 dark:from-purple-900/20 dark:to-purple-800/20">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-1 md:space-x-2">
                <button
                  ...
                  onClick={(e) => handleViewChange("month", e)}
                  onMouseDown={(e) => e.preventDefault()}
                  className="px-2 py-1 text-xs md:text-sm font-semibold text-purple-700 dark:text-purple-300 hover:bg-white/30 dark:hover:bg-white/10 rounded-lg transition-colors"
                >
              ...
              <div className="flex items-center space-x-1">
                <button
                  ...
                  onMouseDown={(e) => e.preventDefault()}
                  className="p-1 text-purple-700 dark:text-purple-300 hover:bg-white/30 dark:hover:bg-white/10 rounded-lg transition-colors"
                >
                  <svg
          ...
          {/* Calendar Body - Responsive height without scroll (dropdown handles overflow) */}
          <div
            className="flex-1 p-1 md:p-2 flex flex-col"
            style={{
              height: `${dropdownPosition.height - 60}px`, // Responsive height: total height - 60px header
            }}
          >
            {view === "calendar" && (
              <div className="h-full flex flex-col">
                {/* Day Headers */}
                <div className="flex-shrink-0 grid grid-cols-7 gap-1 mb-3">
                  {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map(
                    (day) => (
                      <div
                        key={day}
                ...
                {/* Calendar Grid - Flexible with better spacing and larger buttons */}
                <div className="flex-1 grid grid-cols-7 gap-1 content-start p-1">
                  {generateCalendarDays(currentDate).map((date, index) => (
                    <button
                      type="button"
                      key={index}
                      ...
                      disabled={!date || isDateDisabled(date)}
                      className={`
                        aspect-square text-sm rounded-lg transition-all duration-200 flex items-center justify-center font-medium min-h-[25px] max-h-[32px] w-full
                        ${!date ? "invisible" : ""}
                        ${isToday(date!) && !isSelected(date!) ? "bg-purple-100 dark:bg-purple-900/30 text-purple-600 dark:text-purple-300 ring-2 ring-purple-300 dark:ring-purple-700" : ""}
                        ${isSelected(date!) ? "bg-purple-500 text-white shadow-lg transform scale-105 ring-2 ring-purple-300" : ""}
                        ${!isSelected(date!) && !isToday(date!) && date && !isDateDisabled(date!) ? "hover:bg-purple-50 dark:hover:bg-purple-900/20 text-gray-700 dark:text-gray-200 hover:text-purple-600 dark:hover:text-purple-300 hover:scale-105" : ""}
                        ${isDateDisabled(date!) ? "text-gray-500 dark:text-gray-600 cursor-not-allowed opacity-50" : ""}
                        ${date && date.getMonth() !== currentDate.getMonth() ? "text-gray-400 dark:text-gray-600 opacity-60" : ""}
                      `}
                    >
            ...
            {/* Month Selection */}
            {view === "month" && (
              <div className="grid grid-cols-3 gap-3 h-full content-start p-0.5 mt-5">
                {monthRange.map((month) => {
                  const currentYear = new Date().getFullYear();
                  const currentMonth = new Date().getMonth();
                  const isCurrentOrFutureMonth =
                  ...
                  return (
                    <button
                      ...
                      disabled={!isCurrentOrFutureMonth}
                      className={`
                        px-3 py-3 text-sm rounded-lg transition-all duration-200 font-medium
                        ${currentDate.getMonth() === month ? "bg-purple-500 text-white shadow-md transform scale-105" : ""}
                        ${currentDate.getMonth() !== month && isCurrentOrFutureMonth ? "hover:bg-purple-50 dark:hover:bg-purple-900/20 text-gray-700 dark:text-gray-200 hover:text-purple-600 dark:hover:text-purple-300 hover:scale-105" : ""}
                        ${!isCurrentOrFutureMonth ? "text-gray-300 dark:text-gray-600 cursor-not-allowed opacity-50" : ""}
                      `}
                    >
              ...
              <div className="grid grid-cols-3 gap-3 h-full content-start p-0.5 mt-5">
                {yearRange.map((year) => (
                  <button
                    ...
                    onMouseDown={(e) => e.preventDefault()}
                    className={`
                      px-3 py-2 text-sm rounded-lg transition-all duration-200 font-medium flex-shrink-0
                      ${currentDate.getFullYear() === year ? "bg-purple-500 text-white shadow-md transform scale-105" : "hover:bg-purple-50 dark:hover:bg-purple-900/20 text-gray-700 dark:text-gray-200 hover:text-purple-600 dark:hover:text-purple-300 hover:scale-105"}
                    `}
                  >
                    {year}
                  </button>
