...
const calculateGrowthRate = (current: number = 0, previous: number = 0): number => {
  if (previous === 0) return current > 0 ? 100 : 0;
  return parseFloat(((current - previous) / previous * 100).toFixed(2));
};

const DataStatsOne: React.FC<{ stats?: StatsProps }> = ({ stats }) => {
  // Return spinner if stats is not loaded yet
  if (!stats) {
    return (
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3 md:gap-6  mt-1">
        {[...Array(3)].map((_, index) => (
  ...
  const statsData = [
    {
      ...
      color: "#3FD97F",
      title: "Bookings (last 30 days)",
      value: stats.bookings?.current?.toString() || "0",
      growthRate: calculateGrowthRate(stats.bookings?.current, stats.bookings?.previous),
    },
    {
      icon: (
        <svg width="26" height="26" viewBox="0 0 26 26" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      growthRate: calculateGrowthRate(stats.revenue?.current, stats.revenue?.previous),
    },
    // {
    //   icon: (
    //     <svg width="26" height="26" viewBox="0 0 26 26" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    //   color: "#8155FF",
    //   title: "Campaigns (last 30 days)",
    //   value: (stats.campaigns?.current || 0).toString(),
    //   growthRate: calculateGrowthRate(stats.campaigns?.current, stats.campaigns?.previous),
    // },
    {
      icon: (
        <svg width="26" height="26" viewBox="0 0 26 26" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      color: "#18BFFF",
      title: "Patients (last 30 days)",
      value: (stats.patients?.current || 0).toString(),
      growthRate: calculateGrowthRate(stats.patients?.current, stats.patients?.previous),
    },
  ];
