...
const LogoutModal = ({ loading, isOpen, onClose, onLogout }: LogoutModalProps) => {
  ...
  const [loadingi, setLoadingi] = useState<boolean>(false); // Track loading state
 const loaderImpliment =()=>{
  onLogout()
  setLoadingi(true)
  }
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} backdrop={"blur"}>
      <ModalContent>
        ...
        <ModalFooter>
          <Button color="default" onPress={onClose} className="hover:bg-red-500">
            Cancel
          </Button>
          <Button color="primary" onPress={loaderImpliment} isDisabled={loading}>
             <p>OK </p>
            <p>{loadingi && <Spinner size="sm"  color="white"/> }</p>
          </Button>
        </ModalFooter>
      </ModalContent>
    </Modal>
